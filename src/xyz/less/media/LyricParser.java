...
public final class LyricParser {
	...
	public static final String TIME_REGEX = "\\d{2}:\\d{2}(:\\d{2})?(\\.\\d{2,3})?";
	public static final String TIME_LINE_REGEX = "^\\[" + TIME_REGEX + "].*";
	...
	private boolean isValidTime(String time) {
		return StringUtil.trim(time).matches(TIME_REGEX);
	}

	private boolean isTimeDataTag(String line) {
//		return Character.isDigit(line.charAt(1));
		return StringUtil.trim(line).matches(TIME_LINE_REGEX);
	}
